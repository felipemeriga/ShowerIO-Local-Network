...
const {values, key, str, not, by, grep, lo, is, debounce, extend, falsy, send, file, noop, update, identity, time, includes} = require('utilise/pure')
    ...
    , rijs = opts =
>
require('rijs.npm')(require('rijs')(opts))
...
const heartbeat = vm =
>
setInterval(d = > vm.eval('', e = > {if(e) console.error(e)
}),
30000
)

const canonical = farm =
>
browser =
>
is.str(browser)
...
const emitReload = socket =
>
socket.send(stringify({data: {exec: () = > location.reload()}
}))
